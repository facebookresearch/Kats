...
from infrastrategy.kats.detectors.bocpd_onedetection import BocpdDetectorModel
from infrastrategy.kats.detectors.cusum_detection import (
    CUSUMDetector,
    CUSUMDetectorModel,
    CusumScoreFunction,
    MultiCUSUMDetector,
...
from infrastrategy.kats.detectors.outlier import (
    MultivariateAnomalyDetector,
    MultivariateAnomalyDetectorType,
    OutlierDetector,
)
from infrastrategy.kats.detectors.prophet_detector import ProphetDetectorModel
from infrastrategy.kats.detectors.robust_stat_detection import RobustStatDetector
from infrastrategy.kats.detectors.seasonality import ACFDetector, FFTDetector
from infrastrategy.kats.detectors.slow_drift_detector import SlowDriftDetectorModel, time_series_to_data_points
from infrastrategy.kats.detectors.stat_sig_detector import (
    MultiStatSigDetectorModel,
    StatSigDetectorModel,
)
from infrastrategy.kats.detectors.trend_mk import MKDetector
from infrastrategy.kats.detectors.window_slo_detector import WindowSloDetectorModel
...
daily_data = pd.read_csv("infrastrategy/kats/data/peyton_manning.csv")
daily_data.columns = ["time", "y"]
ts_data_daily = TimeSeriesData(daily_data)

DATA_multi = pd.read_csv("infrastrategy/kats/data/cdn_working_set.csv")
...
class MultiCUSUMDetectorTest(TestCase):
    def test_gaussian_increase(self):
        ...
        df_increase = pd.DataFrame(
            np.concatenate(
                    [np.random.multivariate_normal(mean1, sigma, 60), np.random.multivariate_normal(mean2, sigma, 30)]
        ))
    ...
    def test_gaussian_decrease(self):
        ...
        df_decrease = pd.DataFrame(
            np.concatenate(
                    [np.random.multivariate_normal(mean2, sigma, 60), np.random.multivariate_normal(mean1, sigma, 30)]
        ))
...
class TestCUSUMDetectorModel(TestCase):
    ...
    def test_streaming(self):
        ...
        anomaly_score = TimeSeriesData(time=pd.Series(), value=pd.Series([], name="ts_value"))
...
class TestSlowDriftDetector(TestCase):
    def generate_ts_with_slope_and_seasonality(
        ...
        for i in range(num_points):
            ...
            values.append(value)
        return TimeSeriesData(time=pd.Series(times), value=pd.Series(values), use_unix_time=True, unix_time_units="s")
