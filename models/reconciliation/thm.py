...
from infrastrategy.kats.models.reconciliation.base_models import (
    BaseTHModel,
    calc_mape,
    calc_mae,
    GetAggregateTS,
...
class TemporalHierarchicalModel:
    ...
    data: TimeSeriesData
        Training timeseries data for level 1 (i.e., the most disaggregate level).
    baseModels: List[BaseTHModel]
        List of base models for different levels.
    ...
    def _get_m(self, ks: List[int]) -> int:
        """
        Calculate m, which is the minimum common multiple of all levels.
        :Parameters:
        ks: List[int]
            All integers (i.e., levels) for calculation.
        :Returns:
    ...
    def predict(
        ...
        """
        Generate forecasts (with time index).
        ...
        methd: str = 'struc'
            Reconciliation method.
        freq: Optional[str] = None
            Frequency for level 1. If not given, then infer the frequency via ts.infer_freq_robust().
        origin_fcst: bool = False
            Whether to return the forecasts of base models.
        fcst_levels: Optional[List[int]] = None
            Levels that one wants to generate forecasts for.
    ...
    def median_validation(
        ...
        """
        Filtering out bad fcsts based on median fcsts

        :Parameters:
        steps: int
            Number of forecasts for level 1 for validation.
        dist_metric: str = 'mae'
            Distance matric used to measure the distance between base forecasts and corresponding median forecasts.
        threshold: float = 3
            Forecast with distance >= median+threshold*std would be taken as bad forecasts.

        :Returns:
        List of levels of bad forecasts.
        """
        diffs = {}
        ks = self.levels
        if dist_metric == "mae":
