...
class ARNetParams(Params):
    """Specifies parameters for the model.
...
class ARNet(m.Model):
    ...
    def predict(self, steps, **kwargs):
        """Predict method with AR_Net model

        Given a train model, makes a prediction using
        the previously loaded data.

        Args:
            steps: Number of steps to make the forecast for.
        """
    ...
    def plot(self):
        """
        Provides a plot for the forecast made by the AR-Model.
        """
    ...
    def get_parameter_search_space() -> List[Dict[str, object]]:
        """Provides a parameter space for AR-net to run bayesian
        optimization over.

        For nowMove the implementation of get_parameter_search_space() out of ar_net
