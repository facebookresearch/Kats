...
from kats.models.nowcasting import feature_extraction
...
class TsFeatures:
    ...
    def get_nowcasting(
        ...
        if extra_args.get("nowcast_ma", default_status):
            nowcasting_features["nowcast_ma"] = np.nan
        if extra_args.get("nowcast_mom", default_status):
            nowcasting_features["nowcast_mom"] = np.nan
        if extra_args.get("nowcast_lag", default_status):
            nowcasting_features["nowcast_lag"] = np.nan
        if extra_args.get("nowcast_macd", default_status):
            nowcasting_features["nowcast_macd"] = np.nan
        if extra_args.get("nowcast_macdsign", default_status):
            nowcasting_features["nowcast_macdsign"] = np.nan
        if extra_args.get("nowcast_macddiff", default_status):
            nowcasting_features["nowcast_macddiff"] = np.nan
