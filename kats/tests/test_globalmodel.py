...
class GMModelTest(TestCase):
    def test_model(self):
        ...
        gmmodel.save_model("gmm_test.p")
        gmmodel2 = load_gmmodel_from_file("gmm_test.p")
        test_tensor = torch.randn((2, 177))

        gmmodel.rnn.reset_state()
        tensor1 = gmmodel.rnn(test_tensor)
        gmmodel2.rnn.reset_state()
        tensor2 = gmmodel2.rnn(test_tensor)

        self.assertTrue(
            torch.all(tensor1 == tensor2),
            "The results generated from the loaded model is different from that generated by the original model.",
        )
